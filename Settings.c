...
static void Settings_readFields(Settings* settings, Hashtable* dynamicColumns, const char* line) {
   char* trim = String_trim(line);
   char** ids = String_split(trim, ' ', NULL);
   free(trim);
   ...
   for (j = 0, i = 0; ids[i]; i++) {
      ...
      if (sscanf(ids[i], "Dynamic(%30s)", dynamic)) {
         ...
         *end = '\0';
         unsigned int key;
         if (!DynamicColumn_search(dynamicColumns, dynamic, &key))
            continue;
      ...
      int id = atoi(ids[i]) + 1;
      if (id > 0 && id < LAST_STATIC_PROCESSFIELD && Process_fields[id].name) {
...
static bool Settings_read(Settings* this, const char* fileName, unsigned int initialCpuCount, Hashtable* dynamicColumns) {
   ...
   for (;;) {
      ...
      if (String_eq(option[0], "fields")) {
         Settings_readFields(this, dynamicColumns, option[1]);
         didReadFields = true;
...
int Settings_write(const Settings* this, Hashtable* dynamicColumns, bool onCrash) {
   ...
   writeFields(fd, this->fields, dynamicColumns, "fields");
   // This "-1" is for compatibility with the older enum format.
...
Settings* Settings_new(unsigned int initialCpuCount, Hashtable* dynamicColumns) {
   ...
   #endif
   this->fields = xCalloc(LAST_STATIC_PROCESSFIELD + 1, sizeof(ProcessField));
   ...
   bool ok = false;
   if (legacyDotfile) {
      ok = Settings_read(this, legacyDotfile, initialCpuCount, dynamicColumns);
      if (ok) {
         // Transition to new location and delete old configuration file
         if (Settings_write(this, dynamicColumns, false) == 0) {
   ...
   if (!ok) {
      ok = Settings_read(this, this->filename, initialCpuCount, dynamicColumns);
   }
   if (!ok) {
      this->changed = true;
      ok = Settings_read(this, SYSCONFDIR "/htoprc", initialCpuCount, dynamicColumns);
   }
   if (!ok) {
      Settings_defaultMeters(this, initialCpuCount);
